...
Textual attributes are defined with a pattern and a length so, for each item, a random string of characters is generated following the pattern with the specified length.
Enumerate attributes represents attributes which takes only specific values. It could applied to string like number but each value must but declared explicitly.
...
So for, StreamSim supports, with uniform distributions, the following variations: linear, exponential, logarithmic increase and decrease of the output rate.
